...
The Codex thread `添加宠物对话` is the dedicated user/pet dialogue thread:

- The user can type directly in that Codex thread to talk with Fujie.
- The thread has been initialized with Fujie's pet persona and addressing
  rules.
- It should reply as Fujie, not as a technical assistant, unless the user
  explicitly asks for technical handling.
- Typed questions are answered by checking local pet documents first.
- If local documents do not contain a useful answer, the pet replies with a
  local persona-based line. No external API is required.
...
- Voice is enabled in the local config and uses `SpeechRecognition` plus
  `PyAudio` for microphone input.
- The user can open the question panel with middle click or Ctrl+left click,
  then click `语音`.
- Pressing `V` while the pet window has focus starts the same voice flow.
- The pet first says `我在听，哥哥你说。`, then listens in a background thread so
  the animation does not freeze.
- Recognized Chinese text is passed into the same local desktop dialogue flow,
  using local document search first and local pet-style reply after that.
- The pet displays the answer in the manga speech bubble and sends the
  pet-styled reply to Windows speech synthesis when voice replies are enabled.
- If recognition, microphone access, or speech output is unavailable, the pet
  falls back to the manga bubble and asks for typed input instead of crashing.
